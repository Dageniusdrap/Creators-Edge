import type { AnalysisResult, BrandVoice, SocialPlatform } from '../types';

export const formatBrandVoice = (brandVoice: BrandVoice): string => `
...
export const getLiveSessionDebriefPrompt = (transcript: string): string => `
...
export const getThumbnailAnalysisPrompt = (): string => `
...
export const getBrainstormIdeasPrompt = (keyword: string): string => `
...
export const getRepurposeContentPrompt = (content: string): string => `
...
export const getDocumentPrompt = (script: string, description: string): string => `
...
export const getSalesCallPrompt = (script: string, brandVoice: BrandVoice, hasFile: boolean): string => {
    const basePrompt = `
...
export const getSocialMediaPrompt = (script: string, description: string, link: string, brandVoice: BrandVoice): string => `
...
export const getProductAdPrompt = (script: string, description: string, link: string, brandVoice: BrandVoice): string => `
...
export const getFinancialReportPrompt = (script: string, description: string): string => `
...
export const getLiveStreamPrompt = (): string => `
...
export const getABTestPrompt = (contentA: string, contentB: string): string => `
...
export const getBrandVoiceScorePrompt = (text: string, brandVoice: BrandVoice): string => `
...
export const getImprovedContentPrompt = (result: AnalysisResult, brandVoice: BrandVoice): string => {
    const originalContent = result.transcript?.map(t => t.text).join('\n') || result.documentAnalysis?.summary || '';
    const feedback = result.feedbackCard
        ? `Strengths: ${result.feedbackCard.strengths?.join('. ') || ''}. Opportunities: ${result.feedbackCard.opportunities?.join('. ') || ''}.`
...
export const getSocialPostPrompt = (result: AnalysisResult, platform: SocialPlatform): string => {
...
export const getYouTubePostPrompt = (result: AnalysisResult, brandVoice: BrandVoice): string => {
    const content = result.transcript?.map(t => t.text).join('\n') || result.documentAnalysis?.summary || '';
...
export const getProductAdScriptPrompt = (result: AnalysisResult): string => {
    const content = result.transcript?.map(t => t.text).join('\n') || result.documentAnalysis?.summary || '';
...
export const getKeyTakeawaysPrompt = (result: AnalysisResult): string => {
    const content = result.transcript?.map(t => t.text).join('\n') || result.documentAnalysis?.summary || '';
...
export const getDescriptionPrompt = (result: AnalysisResult, brandVoice: BrandVoice): string => {
    const content = result.transcript?.map(t => t.text).join('\n') || result.documentAnalysis?.summary || '';
...
export const getViralScriptPrompt = (prompt: string, link: string, brandVoice: BrandVoice): string => `
...
export const getSocialPostFromScriptPrompt = (script: string, brandVoice: BrandVoice): string => `
...
export const getMonetizationAssetsPrompt = (result: AnalysisResult, brandVoice: BrandVoice): string => {
    const content = result.transcript?.map(t => t.text).join('\n') || result.documentAnalysis?.summary || '';
    return `
...
export const getRefineTextPrompt = (text: string, instruction: string): string => `
...
export const getRetirementPlanPrompt = (inputs: any): string => `
    Act as a financial planner specializing in creator economics.
    Create a comprehensive retirement plan based on the following inputs:
    ...
    Calculate the projected nest egg and monthly income.
    Provide a feasibility assessment, summary, and phases for accumulation and decumulation.
    Generate a projection array for every 5 years.
    
    Your response MUST be a single JSON object conforming to the 'retirementPlanSchema'.
`;